...
else
{
	...
	// Читаем кто лучше сыграл ----------------------------- Если есть ктото лучше вставляем его данные
		$result = f_mysqlQuery ("SELECT id_user, users.login, score FROM games_".$theme."_com AS tb, users
					WHERE id_game=".$canvasState." AND id_user=users.id
					ORDER BY score DESC, xod, tb.data, tb.time LIMIT 1;");
		if (mysql_num_rows($result) == 0) $text = "Игра уже удалена.";
		else {
			$data=mysql_fetch_row($result);
	// Кто открыл игру
			$data_=mysql_fetch_row(f_mysqlQuery ("SELECT id_user FROM games_".$theme."_com WHERE id_game=".$canvasState." ORDER BY id LIMIT 1;"));
			if ($_SESSION["id"] != $data_[0])
			{
				if ($_SESSION["id"] != $data[0])
				{
					$data=mysql_fetch_row(f_mysqlQuery ("SELECT gameboard FROM games_".$theme." WHERE id_game=".$canvasState.";"));
					$text .= $data[0];
				}
